	...
	public static void main(String[]args) throws InterruptedException, IOException{
		 try{
		     ...
		     int year = 2010;
		      ...
		      String yearhref = debatesYearLinks.getElement(6).getAt("href");
		      System.out.println("the href of year 2015-2016 is " + yearhref);
		      
		      userAgent.visit(yearhref);
		      Elements mpsList1 = userAgent.doc.findEvery("<p class=\"style1\">"); //list with Groups on MP names sorted by letters
		      Element mpsList = mpsList1.getElement(1);
		    ...
		    //for(int k = 19; k < 26; k++){  //size 24 for each alphabet
		      //for(int k = 0; k < 5; k++){
		      Elements mpNameList = mpsList.findEach("<a href");
		      Thread.sleep(5000);
		      //start of id="loopC"
		      System.out.println("this is the number of MPs " + mpNameList.size());
		      for(int i=500; i < mpNameList.size(); i++){
		     ...
		     String mpPartyName = userAgent.doc.findEvery("<p").getElement(0).getText(); //gets the name of the MPs party to which he belongs
		     ...
		     //csvWriter(polParty);
		     String getMPSubjectsPageHref = userAgent.doc.findEvery("<p>").getElement(9).getFirst("<a").getAt("href"); //gets the href for the subject to which the MP spoke about
		     ...
		     Elements mpSubjectsList =  userAgent.doc.findEvery("<p class=\"eightpointtext\">").getElement(2).findEach("<a href"); //list of all subjects that the mp spoke of
		     Elements mpSubjectSpokenDateList= userAgent.doc.findEvery("<p class=\"eightpointtext\">");
		    ...
		    for(int j =0; j <mpSubjectsList.size(); j++ ){
		     //for(int j = 42 ; j <mpSubjectsList.size(); j++ ){
		     String mpSubjectSpokenDate =  mpSubjectSpokenDateList.getElement(2).getElement(j).getText().trim();
		    	...
		    	if(opinionHolder.contains(mpLastName)){
		    		...
		    		if(opinionHolder.contains(mpLastName)){
		    			...
		    			System.out.println("the opinion holder is " + opinionHolder + " and they said "+ debate.getText());
				    		...
				    		primaryName = opinionHolder;
				    		//System.out.println("if b tage = 1 " + primaryName);
				    	}
		    	}else if(bTagSize == 0){
